// Package rubygems implements the hermetic build recipe for RubyGems.
package rubygems
...
const gemsBase = "https://rubygems.org/gems"

// Recipe downloads and verifies .gem files.
type Recipe struct{}

// New creates a new RubyGems Recipe.
func New() *Recipe { return &Recipe{} }

// Ecosystem implements core.BuildRecipe.
func (r *Recipe) Ecosystem() string { return "rubygems" }

// Build downloads the .gem file and verifies its SHA256.
func (r *Recipe) Build(ctx context.Context, src core.SourceArtifact, _ core.Sandbox) (core.BuiltArtifact, error) {
	...
	gemURL := fmt.Sprintf("%s/%s-%s.gem", gemsBase, name, version)
	if src.Package.SourceURL != "" {
		gemURL = src.Package.SourceURL
	}

	dlPath, sha256sum, err := util.Download(gemURL, ".gem")
	if err != nil {
		return core.BuiltArtifact{}, fmt.Errorf("rubygems build: download: %w", err)
	}
	defer os.Remove(dlPath)

	var log strings.Builder
	fmt.Fprintf(&log, "=== rubygems build: %s@%s ===\n", name, version)
	fmt.Fprintf(&log, "gem_url:             %s\n", gemURL)
	...
	if src.Package.Checksum != "" && sha256sum != src.Package.Checksum {
		return core.BuiltArtifact{}, fmt.Errorf("rubygems build: sha256 mismatch for %s@%s", name, version)
	}

	outPath := util.StableOutput("rubygems", name, version, ".gem")
	if err := util.CopyFile(dlPath, outPath); err != nil {
		return core.BuiltArtifact{}, fmt.Errorf("rubygems build: copy: %w", err)
	}
